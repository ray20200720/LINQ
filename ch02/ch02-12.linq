...
void Main()
{
	...
	var q = from call in callLog
		...
		select new { contact.FirstName, contact.LastName, call.When, call.Duration };
	
	
	foreach(var c in q)
	{
		Console.WriteLine("{0} - {1} {2} ({3} min)", c.When.ToString("ddMMM HH:m"), c.FirstName, c.LastName, c.Duration);
	}
}
